...
namespace Fusion.Summary.Api.Controllers.Filter;

/// <summary>
///     Enables support for overriding the HttpContext.User object with a claims principal constructed using a provided
///     user identifier.
...
public class EmulatedUserSupport : ActionFilterAttribute
{
    public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        {
            var authResult = await context.HttpContext.Request.RequireAuthorizationAsync(r => { r.AlwaysAccessWhen().ResourcesFullControl(); });
...
public static class RequestExtensions
{
    ...
    public static async Task<ClaimsPrincipal> GetEmulatedClaimsUserAsync(this HttpContext httpContext, string userId)
    {
        ...
        if (userProfile == null)
        {
            throw new InvalidOperationException($"Unable to resolve user profile for {userId}, unable emulate user access.");
        }
