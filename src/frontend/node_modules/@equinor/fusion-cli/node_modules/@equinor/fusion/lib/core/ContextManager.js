...
export default class ContextManager extends ReliableDictionary {
    ...
    getHistory() {
        const value = this.toObject();
        return (value === null || value === void 0 ? void 0 : value.history) || [];
    }
...
const filterContextByContextType = (validContextTypes, contexts) => contexts.filter((context) => validContextTypes.includes(context.type.id));
const useContextHistory = () => {
    const contextManager = useContextManager();
    const validContextTypes = useCurrentContextTypes();
    const [history, setHistory] = useState(filterContextByContextType(validContextTypes, contextManager.getHistory()));
    const setHistoryFromCache = useCallback((contextCache) => {
        if (contextCache.history !== history) {
            setHistory(filterContextByContextType(validContextTypes, contextCache.history || []));
        }
    }, [validContextTypes]);
    useEffect(() => {
...
const useContextQuery = () => {
    ...
    const filterContexts = (_b = (_a = useCurrentApp()) === null || _a === void 0 ? void 0 : _a.context) === null || _b === void 0 ? void 0 : _b.filterContexts;
    ...
    const fetchContexts = useCallback(async (query) => {
        if (canQueryWithText(query)) {
            ...
            try {
                ...
                setContexts((filterContexts === null || filterContexts === void 0 ? void 0 : filterContexts(response.data)) || response.data);
                setIsQuerying(false);
            }
            catch (e) {
                setError(e);
