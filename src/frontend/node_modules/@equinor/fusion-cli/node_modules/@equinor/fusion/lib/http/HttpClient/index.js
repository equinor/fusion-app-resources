...
const parseHeaders = (input) => input
    .trim()
    .split(/[\r\n]+/)
...
export default class HttpClient {
    ...
    async headAsync(url, init, responseParser) {
        init = ensureRequestInit(init, (init) => (Object.assign(Object.assign({}, init), { method: 'HEAD' })));
        const response = await this.performFetchAsync(url, init);
        return await this.parseResponseAsync(init, response, responseParser);
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            xhr.upload.addEventListener('error', () => {
                ...
                if (response) {
                    ...
                    reject(new HttpClientRequestFailedError(url, xhr.status, errorResponse, headers));
                }
    ...
    async performFetchAsync(url, init, retryTimeout = 3000) {
        try {
            ...
            if (!response.ok) {
                ...
                throw new HttpClientRequestFailedError(url, response.status, errorResponse, response.headers);
            }
            return response;
        }
        catch (error) {
