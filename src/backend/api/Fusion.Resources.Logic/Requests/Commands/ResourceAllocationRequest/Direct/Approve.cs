            ...
            public class Approve : TrackableRequest
            {
                ...
                public class Validator : AbstractValidator<Approve>
                {
                    public Validator(ResourcesDbContext db)
                    {
                        ...
                        {
                            return await db.ResourceAllocationRequests.AnyAsync(y => y.Id == id && y.Type == DbResourceAllocationRequest.DbAllocationRequestType.Direct);
                        }).WithMessage($"Request of type: '{DbResourceAllocationRequest.DbAllocationRequestType.Direct}' must exist to be able to approve.");
                    }
                }
