...
{
    public static class ApiPagedCollection
    {
        public static ApiPagedCollection<TItem> FromCollection<TItem>(IEnumerable<TItem> items, int? totalCount)
        {
            var apiModel = new ApiPagedCollection<TItem>(items, totalCount ?? items.Count());
            return apiModel;
        }
